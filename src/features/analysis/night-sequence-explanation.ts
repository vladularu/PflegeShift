...
/** Calendar-based explanation only; no allocation, entitlement or salary output. */
...
/** Bounded detail-screen computation; never used by annual reports or pay.ts. */
export function explainNightSequence(
  ...
  const until = first.add({ months: 2, days: 1 }).toString();
  ...
  if (!invalid) {
    for (const anchor of qualified) {
      ...
      const monthNight = following.find((night) => night.date.startsWith(`${month}-`));
      const pair = anchor.date.startsWith(`${month}-`)
        ? following.slice(0, 2)
        : monthNight
          ? [monthNight, ...following.filter((night) => night !== monthNight)].slice(0, 2)
          : [];
  ...
  return Object.freeze({
    ...
    deadline,
    uncertain,
    hasAbsence: relevant.some((entry) => entry.type === "VACATION" || entry.type === "SICK"),
  });
}
