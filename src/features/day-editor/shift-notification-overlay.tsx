...
} from "@/infrastructure/notifications/notification-permissions";
import { MOTION } from "@/theme/motion";
import { usePalette } from "@/theme/palette";
import { TEXT_MAX_SCALE } from "@/theme/typography";

const DEFAULT_NOTIFICATION: EntryNotification = {
  amount: 0,
  unit: "MINUTE",
  direction: "BEFORE",
  reference: "START",
...
const PRESETS = [
  ...
  { label: "Zum Ereigniszeitpunkt", value: DEFAULT_NOTIFICATION },
  {
    label: "5 Minuten vor Beginn",
    value: { ...DEFAULT_NOTIFICATION, amount: 5 },
  },
  {
    label: "1 Stunde vor Beginn",
    value: { ...DEFAULT_NOTIFICATION, amount: 1, unit: "HOUR" as const },
  },
  {
    label: "1 Tag vor Beginn",
    value: { ...DEFAULT_NOTIFICATION, amount: 1, unit: "DAY" as const },
  },
...
const AMOUNTS = Array.from({ length: 366 }, (_, amount) => ({
  label: String(amount),
  value: amount,
}));
const UNITS: readonly { readonly label: string; readonly value: NotificationUnit }[] = [
...
const DIRECTIONS: readonly {
  readonly label: string;
  readonly value: NotificationDirection;
}[] = [
...
const REFERENCES: readonly {
  readonly label: string;
  readonly value: NotificationReference;
}[] = [
...
function notificationsEqual(
  first: EntryNotification | null,
  second: EntryNotification | null,
): boolean {
...
function isPreset(value: EntryNotification | null): boolean {
  return PRESETS.some((preset) => notificationsEqual(preset.value, value));
}

function customSummary(value: EntryNotification): string {
  return value.amount === 0 ? "Zum Ereigniszeitpunkt" : notificationLabel(value);
}
...
function CustomNotificationView({
  ...
  readonly draft: EntryNotification;
  readonly onCancel: () => void;
  readonly onChange: (value: EntryNotification) => void;
  ...
  return (
    <Animated.View
      ...
      testID="shift-notification-custom"
    >
      <View style={styles.customHeader}>
        <Pressable
          accessibilityLabel="Benutzerdefinierte Benachrichtigung schließen"
...
export function ShiftNotificationOverlay({
  onChange,
  onClose,
  value,
}: {
  ...
  const [draft, setDraft] = useState<EntryNotification>(value ?? DEFAULT_NOTIFICATION);
  const [selectedValue, setSelectedValue] = useState<EntryNotification | null>(value);
  ...
  const ensurePermission = useCallback(async (): Promise<NotificationPermissionState> => {
    try {
      ...
      if (state === "UNKNOWN") state = await getNotificationPermissionState();
      if (state === "NOT_DETERMINED") state = await requestNotificationPermission();
      setPermission(state);
      return state;
    } catch {
  ...
  const applyValue = useCallback(
    async (nextValue: EntryNotification | null): Promise<NotificationPermissionState | "NONE"> => {
    ...
    [ensurePermission, onChange, onClose],
  );
  ...
  if (view === "CUSTOM") {
    return (
      <CustomNotificationView
        ...
        onChange={setDraft}
        onDone={() => {
          void applyValue(draft).then((state) => {
            if (state === "DENIED") setView("PRESETS");
          });
        }}
      />
    );
  ...
  return (
    <View style={styles.overlay} testID="shift-notification-presets">
      <Animated.View
        entering={FadeIn.duration(MOTION.duration.fast).reduceMotion(MOTION.reduceMotion)}
        exiting={FadeOut.duration(MOTION.duration.fast).reduceMotion(MOTION.reduceMotion)}
      ...
      >
        {permission === "DENIED" ? (
          <View
            ...
            style={[styles.permissionCard, { backgroundColor: palette.surface }]}
          >
            <Text
              maxFontSizeMultiplier={TEXT_MAX_SCALE}
              style={[styles.permissionText, { color: palette.textSecondary }]}
            >
              Benachrichtigungen sind in iOS deaktiviert. Aktiviere sie, damit Erinnerungen
              erscheinen.
            </Text>
            <Pressable
              accessibilityRole="button"
            ...
            >
              <Text style={[styles.settingsLabel, { color: palette.onPrimary }]}>
                iOS Einstellungen
              </Text>
            </Pressable>
          </View>
        ...
        >
          {PRESETS.map((preset) => (
            <PresetRow
              key={preset.label}
              label={preset.label}
              onPress={() => void applyValue(preset.value)}
              selected={notificationsEqual(selectedValue, preset.value)}
            />
          ))}
        </View>
        ...
        <Pressable
          accessibilityRole="radio"
          accessibilityState={{ selected: selectedValue !== null && !isPreset(selectedValue) }}
          onPress={() => {
            setDraft(selectedValue ?? DEFAULT_NOTIFICATION);
            setView("CUSTOM");
          }}
          style={({ pressed }) => [
            styles.customRow,
        ...
        >
          <RadioMark selected={selectedValue !== null && !isPreset(selectedValue)} />
...
const styles = StyleSheet.create({
  ...
  permissionText: { fontSize: 14, lineHeight: 19, textAlign: "center" },
  settingsButton: {
    minHeight: 50,
    alignItems: "center",
