...
const MONTHLY_2026: TariffTable = {
  P7: { 2: 3510.3, 3: 3701.21, 4: 3998.33, 5: 4149.59, 6: 4305.4 },
  P8: { 2: 3701.21, 3: 3862.8, 4: 4075.58, 5: 4247.92, 6: 4488.98 },
  P9: { 2: 3992.39, 3: 4184.4, 4: 4312.38, 5: 4558.59, 6: 4662.42 },
  P10: { 2: 4184.4, 3: 4312.38, 4: 4675.42, 5: 4850.63, 6: 4960.96 },
  P11: { 2: 4419.71, 3: 4557.3, 4: 4901.27, 5: 5129.69, 6: 5233.54 },
  ...
  P15: { 2: 5132.29, 3: 5293.23, 4: 5695.6, 5: 6177.16, 6: 6361.06 },
  P16: { 2: 5240.04, 3: 5415.9, 4: 5983.76, 5: 6645.71, 6: 6937.7 },
};

const HOURLY_2026: TariffTable = {
  P7: { 2: 20.7, 3: 21.83, 4: 23.58, 5: 24.47, 6: 25.39 },
  P8: { 2: 21.83, 3: 22.78, 4: 24.03, 5: 25.05, 6: 26.47 },
  P9: { 2: 23.54, 3: 24.68, 4: 25.43, 5: 26.88, 6: 27.5 },
  P10: { 2: 24.68, 3: 25.43, 4: 27.57, 5: 28.61, 6: 29.26 },
  P11: { 2: 26.06, 3: 26.88, 4: 28.9, 5: 30.25, 6: 30.86 },
  ...
  P16: { 2: 30.9, 3: 31.94, 4: 35.29, 5: 39.19, 6: 40.91 },
};
...
export function getTariffVersion(date: string): TariffVersion | null {
  return (
    TARIFF_VERSIONS.find((version) => version.validFrom <= date && date <= version.validTo) ?? null
  );
}

export function getMonthlyTableAmount(profile: TariffProfile, date: string): number | null {
  return getTariffVersion(date)?.monthly[profile.payGroup][profile.payLevel] ?? null;
}

export function getIndividualHourlyRate(profile: TariffProfile, date: string): number | null {
  return getTariffVersion(date)?.hourly[profile.payGroup][profile.payLevel] ?? null;
}

export function getPremiumHourlyRate(profile: TariffProfile, date: string): number | null {
