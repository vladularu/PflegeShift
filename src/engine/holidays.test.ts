...
describe("German public holidays", () => {
  ...
  it("applies federal-state rules", () => {
    ...
    expect(getPublicHolidays(2017, "NI").some((holiday) => holiday.name === "Reformationstag")).toBe(true);
    expect(getPublicHolidays(2016, "NI").some((holiday) => holiday.name === "Reformationstag")).toBe(false);
  });
  ...
  it("covers every federal state with the nationwide baseline", () => {
    ...
    for (const state of states) {
      expect(getPublicHolidays(2026, state).filter((holiday) => holiday.scope === "NATIONWIDE")).toHaveLength(9);
    }
  });
});
