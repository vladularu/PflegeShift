...
LUNA Shift speichert ausgewählte Ortsnamen, Adressen und Koordinaten verschlüsselt
in der lokalen SQLCipher-Datenbank. Es gibt keinen LUNA-Shift-Server für diese
...
- LUNA Shift liest keine aktuelle GPS-Position und betreibt weder Vordergrund-
...
- LUNA Shift sendet keine Ortsdaten an einen eigenen Server.
...
| Karte in LUNA Shift anzeigen | `react-native-maps` verwendet ohne Google-Provider das native Apple MapKit. Der Kartenbereich und die Zielkoordinaten werden für die Kartendarstellung verarbeitet. | `react-native-maps` verwendet Google Maps. Zielkoordinaten und für den SDK-Betrieb erforderliche technische Daten werden von Google verarbeitet. |
| In Karten-App öffnen         | Nach bewusster Auswahl öffnet LUNA Shift Apple Karten mit Ortsname und Koordinaten oder Google Maps mit Koordinaten.                                                | Nach bewusster Auswahl öffnet LUNA Shift Google Maps mit Koordinaten.                                                                            |
...
LUNA-Shift-Datenhaltung zu unterscheiden.
